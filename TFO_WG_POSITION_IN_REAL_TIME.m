...
while ishandle(h)
   ...
   for i = 1:length(data)-1
 	if ~any(contains(vehicle_list, data{i}(1:6)))
           continue 
    end
    n = n+1;
...
%         data_split = strsplit(data{i});
        ...
        lat = data_split{3};
        lon = data_split{4};
        if ~any(contains(vehicle_list, vehicle))
           continue 
        end
        ...
        if ~isequal(eval([vehicle '.time(end)']),datenum(datetime,'yyyymmddTHHMMSS')) % append if new timestamp
            ...
            plot_asset(eval(vehicle),coul(n,:))        
            legendInfo{n} = eval([vehicle '.name']);
           
   end
    leg =  legend(legendInfo);
    scatter(kelvinwp(:,2),kelvinwp(:,1),175,'ro','filled')
    leg.String{end} = 'Way Points';
    title(['TFO ALL ASSETS; LAST UPDATED ' datestr(now)])
    shg
    pause(10)
